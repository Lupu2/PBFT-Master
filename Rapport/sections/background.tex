...
The code in \autoref{code:asyncawaitex} is the asynchronous function that is responsible for sending a message over the network in our system. In order for the \code{SendMessage} to be marked as an asynchronous function it has a \code{async} modifier. \code{SendMessage} does not return any values, therefore it returns a .Net \code{Task} object. Most of the code in the function is synchronous and this code transforms a message object to byte streams. The last line however is asynchronous where the socket object calls the asynchronous function \code{SendAsync}. As we want to avoid the function returning before the asynchronous operation is finished, the \code{await} operator is used to wait for the asynchronous operation to finish.
