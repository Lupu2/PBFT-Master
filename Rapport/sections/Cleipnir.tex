...
The Cleipnir serialization process follows a graph-like structure. The original object graph that is to be serialized is called a \code{Roots} object. Most data objects usually have some form of system references. In some cases objects also have reference to other objects. In order to successfully persist an object these references need to also be persisted.  In order to accomplish this the \code{Roots} graph object to the persisted object is connected to the graph object through pathways leading to the references that are to also be persisted~\cite[p.~10]{PAPER:PaxosCleipnir}.
