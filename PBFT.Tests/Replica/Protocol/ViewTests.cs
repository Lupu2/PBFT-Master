...
namespace PBFT.Tests.Replica.Protocol
{
    ...
    public class ViewTests
    {
            ...
            public void RedoMessageLeaderTest()
            {
                ...
                var protexec =
                    new ProtocolExecution(serv, 1, sh.ProtocolSubject, redistpm, null, null, sh.NewViewSubject, sh.ShutdownSubject);
            ...
            public async CTask RedoMessageTest(CList<PhaseMessage> oldpreList, Server Serv,
                Source<PhaseMessage> MesBridge, int failNum)
            {
                Console.WriteLine(oldpreList.Count);
                foreach (var prepre in oldpreList)
                {
                    Serv.InitializeLog(prepre.SeqNr);
                    var precert = new ProtocolCertificate(prepre.SeqNr, prepre.ViewNr, prepre.Digest, CertType.Prepared,
                        prepre); //need a way to know request digest and request message
                    var comcert =
                        new ProtocolCertificate(prepre.SeqNr, prepre.ViewNr, prepre.Digest, CertType.Committed);
                    if (!Serv.IsPrimary())
                    {
                        var prepare = new PhaseMessage(Serv.ServID, prepre.SeqNr, prepre.ViewNr, prepre.Digest,
                            PMessageType.Prepare);
                    ...
                    var commes = new PhaseMessage(Serv.ServID, prepre.SeqNr, prepre.ViewNr, prepre.Digest,
                        PMessageType.Commit);
